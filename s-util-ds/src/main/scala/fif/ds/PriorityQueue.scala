...
abstract class PriorityQueue[A: Cmp: Eq, S]
  ...
  type Bounded = PriorityQueue[A, this.Structure] with BoundedContainer[A, this.Structure]
  type Unbounded = PriorityQueue[A, this.Structure] with UnboundedContainer[A, this.Structure]
}